...
public class MainActivity extends Activity{
    ...
    private long deny_size = 20480; //5G 5120 -> 20G
    ...
    protected void onCreate(Bundle savedInstanceState) {
       ...
       /* offline = (ImageButton) findViewById(R.id.offline);
        ...
        efi_backup.setVisibility(View.GONE);
        ...
        */
        ...
        if(choose_partition.equals("")) {
            tvInfo.setText("无合适的默认分区");
        }else{
            tvInfo.setText("默认已选择分区：" + choose_partition);
        }
...
/*
    ...
    private View getDiskView(Disk d) {
        ...
        for (Volume vol : d.getVolumes()) {
            ...
            int isDeny = 0;
            String sdName = vol.getfDisk() + vol.getBlock();
            String showInfo = "";
            TextView tv = new TextView(this);
            if(vol.getUseType() == null){
                showInfo = "Size: "+ vol.getSize() + "MB";
            }else{
                showInfo = vol.getUseType();
            }

            //tv.setText(vol.getBlock() + "\nSize: "+ vol.getSize() + "MB");
            tv.setText(vol.getFsType() + "\n"+ showInfo);
            tv.setPadding(5, 5, 0, 0);
            tv.setTextSize(15);
            tv.setTextColor(Color.BLACK);

            for (int k=0; k < deny_part.size(); k++){
                if (sdName.equals(deny_part.get(k))) {
                    isDeny = 1;
                }
            }

            // < 5G can not click
            if (isDeny == 1 || vol.getSize() < deny_size){
    ...
    private void section_select() {
        ...
        String section_cmd = "fdisk -l /dev/block/sd[a-z]";
        String blkid_cmd = "blkid /dev/block/sd[a-z][0-9]";
        String mount_cmd = "cat /proc/mounts | grep -E '/system|/data' ";
        String block_dev = "/dev/block/";

        String fdisk_info = exec(section_cmd);
        String blkid_info = exec(blkid_cmd);
        String mount_info = exec(mount_cmd);

        String[] section_info = fdisk_info.trim().split("\\n+");
        String[] get_blkid_info = blkid_info.trim().split("\\n+");
        String[] get_mount_info = mount_info.trim().split("\\n+");

        long sectorSize = 512 ;
        List<String> default_part = new ArrayList<>();
        ...
        ArrayList<Volume> mVolumes = new ArrayList<>(); //to get FS type ...

        for (int i = 0; i < get_blkid_info.length; i++) {
            int beginIndexType = 0;
            if (get_blkid_info[i].startsWith("/dev/block/sd") && ((beginIndexType = get_blkid_info[i].indexOf("TYPE=\"")) != -1)){
                String sdName = get_blkid_info[i].substring(0, 15); // /dev/block/sdb1
                String sdType = get_blkid_info[i].substring(beginIndexType + 6, get_blkid_info[i].indexOf("\"", beginIndexType + 6)); //TYPE="ext4"
                Volume v = new Volume();
                v.setfDisk(sdName);
                v.setFsType(sdType);
                if(sdType.equals("vfat") && sdName.substring(sdName.length() - 1).equals("1")){
                    choose_efi = sdName;
                    v.setUseType("ESP");
                }
                if(sdType.equals("ntfs")){
                    default_part.add(sdName);
                }
                mVolumes.add(v);
            }
        }

        for (int i = 0; i < get_mount_info.length; i++) {
            String useType = null;
            if (get_mount_info[i].startsWith("/dev/sd")){
                String[] mount_line = get_mount_info[i].trim().split("\\s+");
                if(mount_line[1].startsWith("/system")){
                    useType = "OPENTHOS SYSTEM";
                }else if(mount_line[1].startsWith("/data")){
                    useType = "OPENTHOS DATA";
                }

                if(useType != null) {
                    String sdName = block_dev + mount_line[0].substring(mount_line[0].length() - 4);
                    deny_part.add(sdName);

                    for (Volume mV : mVolumes) {
                        if (mV.getfDisk().equals(sdName)) {
                            mV.setUseType(useType);

                            if (mV.getUseType() != null && mV.getUseType().equals("ESP")) {
                                choose_efi = mV.getfDisk();
                            }
                        }
                    }
                }
        ...
        for (int i = 0; i < section_info.length; i++) {
            ...
            //get volume partition info
                ...
                //if partition < 5G -> 20G , deny it .
                if ( tmp_size.compareTo(BigInteger.valueOf(deny_size)) < 0){
                ...
                Volume v = new Volume();
                v.setfDisk(block_dev); // /dev/block/sdb
                v.setBlock(fdisk_line[0]); // 1
                ...
                for (Volume mV : mVolumes){
                    if(mV.getfDisk().equals(block_dev + fdisk_line[0])){
                        v.setFsType(mV.getFsType());
                        v.setUseType(mV.getUseType());
                    }
                }

                if (fdisk_line.length > 6 && fdisk_line[5].startsWith("Microsoft") && fdisk_line[6].startsWith("reserved") ) {
                    v.setUseType("MSR");
                }
                if (fdisk_line.length > 5 && fdisk_line[5].startsWith("EFI")) {
                    choose_efi = block_dev + fdisk_line[0];
        ...
        for (int k=0; k < deny_part.size(); k++){
            ...
            for (int j=0; j < default_part.size(); j++) {
                if(default_part.get(j).equals(deny_part.get(k))){
                    default_part.remove(j);
                    Log.v("WinRec", "Default parts remove: " + default_part.get(j));
                }
            }
        }
        ...
        choose_partition = "";
        int miniInt = 0;
        for (int i=0; i < default_part.size(); i++) {
            Log.v("WinRec", "Default parts: " + default_part.get(i));

            int tmpInt = Integer.parseInt(default_part.get(i).substring(default_part.get(i).length() - 1));
            if((i == 0) || (tmpInt < miniInt)){
                miniInt = tmpInt;
                choose_partition = default_part.get(i);
            }
        }
